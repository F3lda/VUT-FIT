...
**/
	...
	if(root == NULL){
		...
		BSTInsert(&root->LPtr, K, Content);
	} else {
		BSTInsert(&root->RPtr, K, Content);
	}
}
...
**/
	...
	//The Biggest in the left tree
	tBSTNodePtr root = (*RootPtr);
	if(root != NULL){
		if(root->RPtr == NULL){
			tBSTNodePtr delete = root;
			PtrReplaced->Key = delete->Key;
			PtrReplaced->BSTNodeCont = delete->BSTNodeCont;
			PtrReplaced->LPtr = delete->LPtr;
			free(delete);
		} else if(root->RPtr->RPtr == NULL){
			tBSTNodePtr delete = root->RPtr;
			PtrReplaced->Key = delete->Key;
			PtrReplaced->BSTNodeCont = delete->BSTNodeCont;
			BSTDelete(&root, delete->Key);
		} else {
	...
	if(root != NULL){
		if(root->Key == K){
			if(root->LPtr == NULL && root->RPtr != NULL){
				tBSTNodePtr delete = root->RPtr;
				root->Key = delete->Key;
				root->BSTNodeCont = delete->BSTNodeCont;
				root->LPtr = delete->LPtr;
				root->RPtr = delete->RPtr;
				free(delete);
			} else if(root->LPtr != NULL && root->RPtr == NULL){
				tBSTNodePtr delete = root->LPtr;
				root->Key = delete->Key;
				root->BSTNodeCont = delete->BSTNodeCont;
				root->LPtr = delete->LPtr;
				root->RPtr = delete->RPtr;
