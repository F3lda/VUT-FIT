...
{
    jQuery.getJSON("./API/index.php?cmd=getAuctionsAttendedByMe&page_number="+page_number+"&item_count="+item_count,
            function (data) {
                console.log(data);
                tab.empty();
...
function generateMenu(tab, item_count, page)
{
    ...
    var pages;
    jQuery.ajax({
        url: './API/index.php?cmd=getAuctionsAttendedByMeCount',
        type: "POST",
        data: null,
        ...
        success: function (response) {
            $('#user_attended_count').append(`<p class="text-secondary">Celkově ${response["item_count"]} zůčastněných aukcí</p>`);
