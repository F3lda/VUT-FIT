...
{
    jQuery.getJSON("./API/index.php?cmd=getAuctionsNavrzena&page_number="+page_number+"&item_count="+item_count,
            function (data) {
                console.log(data);
                tab.empty();
                for (var i = 0; i < Object.keys(data).length-1; i++)
                {
                    ...
                    row += '<td class="a_author_id">' + getUsername(data[i].autor_id) + '</td>';
                    ...
                    row += '<td class="a_licitator">' + changeText(data[i].licitator_id) + '</td>';
...
function generateMenu(tab, item_count, page)
{
    ...
    var pages;
    jQuery.ajax({
        url: './API/index.php?cmd=getAuctionsNavrzenaCount',
        type: "POST",
        data: null,
        ...
        success: function (response) {
            $('#licitator_approve_count').append(`<p class="text-secondary">Celkově ${response["item_count"]} aukcí ve stavu navržena</p>`);
